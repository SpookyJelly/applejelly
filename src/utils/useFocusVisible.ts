...
function focusVisble(node: AllDocument) {
  ...
  const findAncestor = (elem: HTMLElement) => elem.classList.contains(visiableTarget) ? elem.closest(`.${visiableAncestor}`) : null;
  ...
  const removeEventListeners = (event: any) => {
    if (!event.target.nodeName || event.target.nodeName.toLowerCase() !== "html") {
      isFocus = false;
  ...
  node.addEventListener("focus", (event) => {
    if (isFocusable(event.target as any)) {
      if (!isFocus) {
        ...
        if (
          (elTagName === "INPUT" && elemTypes[elType] && !el.readOnly) ||
          (elTagName === "TEXTAREA" && !el.readOnly) || el.isContentEditable
        ) {
  ...
  node.addEventListener("blur", (event: any) => {
    if (isFocusable(event.target)) {
      ...
      if (elem.classList.contains("focus-visible") || elem.hasAttribute("data-focus-visible-added")) {
        needReset = true;
