...
export const useShortcut = (
    ...
    const {
        canBypassFocus = false,
        elementReferer = document,
        isEnabled = true,
    } = options
    const [shortcutAction, setShortcutAction] =
        useState<MousetrapExtended | null>(null)
    ...
    useEffect(() => {
        if (shortcutAction && bindKey && isEnabled) {
            shortcutAction.bind(bindKey, (event: KeyboardEvent) =>
...
export const useLongPress = (key, action, cancelAction, isEnabled = true) => {
    ...
    useEffect(() => {
        if (
            ...
            console.warn(
                'Key, action, and cancelAction must be provided and valid functions.'
            )
            return
        }
