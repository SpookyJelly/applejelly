...
function baseFormattedNumber(
  ...
  const thousandUnit = shouldFormatInBinary ? binaryThousandUnit : basicThousandUnit
  let exponent = Math.floor(Math.log(Math.abs(reference)) / Math.log(thousandUnit));

  if (
    reference >= basicThousandUnit * Math.pow(binaryThousandUnit, exponent) &&
    ...
    exponent > 0 ? (
      exponent = Math.max.apply(Math, [...keys]),
      position = "top"
    ) : (
  ...
  let fractionDigits = Math.min(maxDecimals, 3 * (1 + exponent) - referenceExponent);
  if (position) {
    if (position === 'top') {
      fractionDigits = 0;
    } else if ((fractionDigits = 3 * (1 + exponent) - referenceExponent - 2) > maxDecimals) {
      return {
