...
import { isNumber } from '@daimre/shared'
import { FONT_SIZE } from '@daimre/styles'
import type { ExpandedSize } from '../types'
...
export const isElementOrParentMatchesClasses = (
  ...
  do {
    if (targetClasses.some(className => currentElement.classList.contains(className))) {
      return true;
    }
...
export function getComputedStyleProperty(
  ...
  const targetElement: HTMLElement = typeof element === 'string' ? document.querySelector(element) || document.body : element;
