...
type Element = Document | Window | HTMLElement | React.RefObject<HTMLElement> | null | undefined
    ...
    const newElem = (() => {
      if (element) {
        if (element instanceof Document) {
          ...
          return 'current' in element ? element.current : defaultElement;
        }
      } else {
...
export function useEventListener(
  ...
  useLayoutEffect(() => {
    if (newElem && newElem.addEventListener && !isDisabled) {
      ...
      return () => newElem.removeEventListener(eventType, currentListener, newOptions);
    }
