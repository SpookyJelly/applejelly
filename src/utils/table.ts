...
export function getMinMax(min: string, max: string) {
    ...
    const minValue = isCssValidUnit(min)
        ? `calc(${min} + 2 * var(--table-cell-padding-x))`
        : min
    const maxValue = max
        ? isCssValidUnit(max)
            ? `calc(${max} + 2 * var(--table-cell-padding-x))`
            : max
        : 'auto'
...
export function arrayToObject(array: any[], functor: (arg0: any) => any) {
...
export function combineWithCondition(value: Obj, modifier: (val?: any) => any) {
...
export const createStyleProp =
    (index: number, hasStyle: boolean): any =>
    (props: Obj): Obj => ({
        ...props,
        key: String(index),
...
export function processColumnsWidths(
    ...
    const convertToMinmax = (
        ...
        if (
            ['min-content', 'auto'].includes(value as string) ||
            String(value).startsWith('fit-content')
        ) {
            return value as string
        }
        const formattedMinValue =
            typeof minValue === 'number'
                ? `${Math.max(0, minValue)}px`
                : minValue
    ...
    return keys
        .map((key: string) => {
            const width =
                isValid && !excludedKeys.includes(key)
                    ? formatStyleValue(widths[key])
                    : widths[key]
...
export function determineAction({
    ...
    if (leftIsAuto && rightIsAuto) {
        return leftIsFloating && rightIsFloating
            ? 'redistribute'
            : leftIsFloating
              ? 'modify-right'
              : 'modify-left'
    } else if (leftIsAuto || rightIsAuto) {
        return (leftIsAuto || leftIsFloating) &&
            (rightIsAuto || rightIsFloating)
            ? 'redistribute'
            : leftIsAuto || leftIsFloating
              ? 'modify-right'
              : 'modify-left'
    } else {
        return 'redistribute'
...
export function processColumnResizing(context) {
    ...
    const resizableKeys = keyColumns.filter((key) => {
        ...
        return (
            column && column.columnDef.enableResizing && column.getCanResize()
        )
    })
    ...
    const getFloatingKeys = (keys, widths) => {
        const results = []
        const countMatchingElements = Object.keys(widths).reduce(
            (count, width) => {
                return keys.includes(width) &&
                    isInvalidColumnWidth(widths[width])
                    ? count + 1
                    : count
            },
            0
        ...
        if (countMatchingElements > 1) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (
                    matchingCount &&
                    countMatchingElements - matchingCount - (isMatching ? 1 : 0)
                ) {
                    results.push(key)
                }
    ...
    for (let i = 0; i < resizableKeys.length - 1; i++) {
        ...
        const action =
            resizeBehavior === 'auto' ||
            (resizeBehavior !== 'modify-left' && !isScrollable)
                ? determineAction({
                      leftIsAuto: isLeftAuto,
                      rightIsAuto: isRightAuto,
                      leftIsFloating: isLeftFloating,
                      rightIsFloating: isRightFloating,
                      hasLeftAuto,
                  })
                : 'modify-left'
...
export function processColumnsProperties(config, values) {
    ...
    const getAccWidths = (accWidths, pCounter) => {
        ...
        const counter = accWidths.reduce((acc, width) => {
            if (R.test(/calc/gi, width)) {
                acc = acc - 1
            }
            return acc
        }, pCounter)

        return `calc(${2 * counter} * var(--table-cell-padding-x) + ${accWidths.join(' + ')})`.replace(
            /calc\((\d+\w*)\)/g,
            '$1'
        )
    }
    ...
    for (let keyColumn of keyColumns) {
        const columnIndex = allColumns.findIndex(
            (column) => keyColumn === getColumnId(column)
        )
        ...
        const stickyColumnIdKey =
            `--table-sticky-column-id-${sanitizeString(keyColumn)}` as const
        stickyColumnOffsets[stickyColumnIdKey] = getAccWidths(
            accumulatedWidths,
            paddingCount
        )
...
export function applyColumnStyles(context, values, stylesTarget) {
    if (context && values) {
        ...
        const columnStyles = {
            ...{
                '--columns-widths': processColumnsWidths(
                    keyColumns,
                    values,
                    columnsMinWidths,
                    nonResizableColumns
...
export function countNonFullWidthRows(rows = []) {
    return rows.reduce((count, row) => {
        const isFullWidthRow = row.cells.every(
            (cell) => cell.column.isFullWidth
        )
