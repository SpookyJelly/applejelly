...
export const useCopyAction = (props) => {
    ...
    const keyboardKey = (
        'string' == typeof shortcutOptionsValue.shortcut
            ? shortcutOptionsValue.shortcut
            : shortcutOptionsValue.shortcut[0]
    ).split('+')
    const keyboardShortcut = (
        <KeyboardShortcut size="xs" keys={keyboardKey} variant="knockout" />
    )
    const renderMsg = () => {
    ...
    const handleClick = (event) => {
        ...
        const requestIdleAction = async () => {
            ...
            const copiedValue = await copyClipboard(_value, {
                format: 'text/plain',
                onCopy: (event) => {
                    const finalValue = isFunction(formattedValue)
                        ? formattedValue(_value)
                        : formattedValue
                    //@ts-expect-error : missing types on setData
    ...
    const shortcutOption = {
        ...
        isEnabled:
            hasShortcut &&
            value !== undefined &&
            Boolean(shortcutOptionsValue.elementReferer),
    }
