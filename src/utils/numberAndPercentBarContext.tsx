...
const getColumnWrapperClass = (columnId) =>
    `--column-${sanitizeStringWithUnderscore(columnId)}--is-wrapping`
...
export function useNumberAndPercentBarContext() {
    ...
    return [
        React.useCallback(
            ({ children, isEnabled = true, hasNoUnwrap = false }) => {
...
export function useColumnWrappingContext(columnId) {
    ...
    const { submitValue, hasNoUnwrap, currentValues } = React.useContext(
        NumberAndPercentBarContext
    )

    const isColumnUnwrappable = Boolean(
        columnId && hasNoUnwrap && currentValues[columnId]
    )
    const shouldListenToVisibility = Boolean(
        columnId && submitValue && !isColumnUnwrappable
    )
    const isWrapping =
        false ===
        useVisibilityObserver(visibilityHelperRef, {
            rootRef,
    ...
    React.useEffect(
        function () {
            isColumnUnwrappable ||
                (columnId &&
                    submitValue &&
                    submitValue(columnId, isWrapping, setIsEnforcedWrapping))
        },
        ...
        function () {
            return function () {
                isColumnUnwrappable ||
                    (submitValue &&
                        columnId &&
                        submitValue(columnId, false, setIsEnforcedWrapping))
            }
        },
    ...
    return {
        rootRef,
        enforcedWrappingValue:
            columnId && !isEnforcedWrapping
                ? `var(${getColumnWrapperClass(columnId)}, 0)`
                : undefined,
        visibilityHelperRef,
