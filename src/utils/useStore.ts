...
export const setStoredValue = (storageKey, pathKey, pathValue) => {
  ...
  try {
    ...
    console.warn("failed to store key ".concat(storageKey, ": ").concat(error))
  }
}
...
export function useStore(storeInfo, defaultValue, option = {}) {
  ...
  const getSafeValue = React.useCallback((value) => {
  ...
  React.useEffect(() => {
    if (isEnabled && !isReadOnly) {
      const newValue = propertyValue !== defaultValue ? propertyValue : undefined
