...
export function useIsStuck(isEnabled = true) {
  ...
  React.useEffect(() => {
    ...
    const isStickyPosition = ["sticky", "-webkit-sticky"].includes(getComputedStyle(stickyTarget).position);
    if (!isStickyPosition) return;

    const updateStickyState = () => {
      const ancestor = scrollableAncestorRef.current || (scrollableAncestorRef.current = findScrollableAncestor(stickyTarget) || document.body);
      ...
      let previousOffsetTop = stickyTarget.offsetTop - initialScrollPosition;

      const checkStuckState = () => {
        const currentScrollPosition = getScrollPosition(ancestor);
        const currentOffsetTop = stickyTarget.offsetTop - currentScrollPosition;
        const scrollDistance = Math.abs(currentScrollPosition - initialScrollPosition);
        const offsetDifference = Math.abs(currentOffsetTop - previousOffsetTop);
      ...
      return () => scrollContainer.removeEventListener("scroll", checkStuckState);
    };
