import React, { useRef, useCallback, useState, useEffect, useLayoutEffect, useMemo, RefObject } from 'react'
...
export function useAsComponent(component: keyof JSX.IntrinsicElements | React.ComponentType<any> ) {
  ...
  const callback = useCallback((_props: React.PropsWithChildren<any>) => {
    if (isNull) {
      return null;
    }
    const style = _props.style || propsRef.current.style ? {
    ...
    return React.cloneElement(component, {
      ...propsRef.current,
      ..._props,
      className: classNames(propsRef.current.className, _props.className),
      style
    })
  }, [compType, isNull]);
...
export function useSafeState( initialState?: any ) {
  ...
  const setSaftState = React.useCallback((newState: any) => {
    if(isMount.current) {
      setState(newState)
    }
  }, [isMount])
...
export function useSafeCallback(initialValue)  {
  ...
  return React.useCallback((updatedValue) => {
...
export function useMemoizedComponent(
  ...
  const memoizedCompareFunction = React.useCallback((prevProps: PropAny, nextProps: PropAny) => {
    return compareFunction ? compareFunction(prevProps, nextProps) : false;
  }, [compareFunction]);

  return React.useMemo(() => {
    return shouldMemoize ? React.memo(Component, compareFunction ? memoizedCompareFunction : undefined) : Component;
  }, [shouldMemoize, Component, compareFunction, memoizedCompareFunction]);
};


type Element = React.RefObject<HTMLElement | Document | Window> | HTMLElement | Document | Window | null
export function useElement(
  ...
  useLayoutEffect(() => {
    const resolvedElement = elementRef ?
      elementRef instanceof Document ? document :
      elementRef instanceof Window ? window :
      elementRef instanceof HTMLElement ? elementRef :
      elementRef.current || defaultElement : defaultElement;
...
export function useHashChange(isDisabled = false) {
  const [hash, setHash] = useUpdatableState(document.location.hash.substring(1))
  useEventListener("hashchange", () => {
    setHash(document.location.hash.substring(1))
  }, undefined, !isDisabled)
  return hash
}
...
function resolveElement(
  targetElement: HTMLElement | Document | Window | RefObject<HTMLElement> | null,
...
export function useResolvedElement<T extends HTMLElement>(
  ...
  const [resolvedElement, setResolvedElement] = useState<T | HTMLElement | Document | Window | null>(null);
