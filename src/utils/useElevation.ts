...
export function useElevation(props: Props) {
    ...
    const [isElevationEnabled, setIsElevationEnabled] = useUpdatableState(
        isEnabled && isFeatureEnabled
    )
    const [computedZIndex, setComputedZIndex] = useSafeState()
    const shouldComputeElevation = isEnabled || isElevationEnabled
    const elevationId = useMemo(
        () => (shouldComputeElevation ? count++ : undefined),
        [shouldComputeElevation]
    )

    useEffect(() => {
        if (isElevationEnabled && elevationId !== undefined) {
            const mapKey = {}
            const baseZIndex = legacyElevation
                ? zIndexValues[legacyElevation]
                : 100
            ...
            if (!isCurrentStack) {
                ...
                elevationValue =
                    lastStack === null || lastStack === undefined
                        ? undefined
                        : lastStack.elevation
            }

            const maxElevation = elevationValue != null ? elevationValue : -1 // null && undefined
            const calculatedElevation = isCurrentStack
                ? elevationValue
                : Math.max(maxElevation + 1, baseZIndex) + size
